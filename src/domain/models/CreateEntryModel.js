  ...
  data: Joi.alternatives()
    .when('type', {
      ...
      then: Joi.object().keys({
        max: Joi.string().max(50).required(),
        med: Joi.string().max(50).required(),
        min: Joi.string().max(50).required(),
        md5name: Joi.string().max(50).required(),
        ext: Joi.string().max(4).required(),
        nameOriginal: Joi.string().max(255).required(),
